...
listValueFiller guiCtx@GUICtx{guiResults =
                                   GUIRes{resValue = txtValue}} vs =
    do
        debugIO "List Value Filler starting..."
        textCtrlAppendText txtValue "["
        listValueFiller' "" guiCtx vs
        textCtrlAppendText txtValue "]"

listValueFiller' :: String -> GUIContext -> [String] -> IO ()
listValueFiller' _ _ [] = return ()
listValueFiller' comma guiCtx@GUICtx{guiResults =
                                         GUIRes{resValue = txtValue}} (val:vs) =
    do
        debugIO "List Value Filler running for a value..."
        textCtrlAppendText txtValue comma
        valueFill guiCtx val >>= textCtrlAppendText txtValue
        listValueFiller' ", " guiCtx vs
