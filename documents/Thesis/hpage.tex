...
\subparagraph{}Para la segunda (y a los fines de este tutorial, definitva) versi'on de \texttt{bestColumn}, experta jugadora de 4 en l'inea como es, pone un poco de criterio y decide que la funci'on tenga en cuenta las condiciones que enumeramos a continuaci'on.  La nueva versi'on de \texttt{bestColumn} y sus funciones adicionales las podemos observar en la Figura \ref{tut208}.  El resto del m'odulo permanece tal como estaba.
\begin{itemize}
...
%%TODO: CONTINUAR DESDE ACç - HAY QUE DEFINIR LAS FUNCIONES EN TƒRMINOS DE tryDropIn
\end{itemize}
\begin{figure}[h]
	\begin{center}
		\begin{lstlisting}
...
bestColumn =
    lift (head . catMaybes) [columnWhereWins p b,
                             columnWhereLoses p b,
                             column3ifAvailable b,
                             firstAvailableColumn b]

--TODO: columnWhereWins

--TODO: columnWhereLooses

column3ifAvailable :: Monad m => HFiaRT m (Maybe Int)
column3ifAvailable = board >>= \b -> return . case length (b !! 3) of
                                                    7 -> Nothing
                                                    _ -> Just 3

firstAvailableColumn :: Monad m => HFiaRT m (Maybe Int)
firstAvailableColumn = board >>= return . Just . length . takeWhile (\c -> length c == 7)
		\end{lstlisting}
