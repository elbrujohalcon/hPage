...
\paragraph{}Gracias, en parte, a haber separado la l'ogica correspondiente a la interfaz de usuario y la propia de \hpage, hemos podido desarrollar esta 'ultima utilizando la t'ecnica de \htmladdnormallinkfoot{TDD}{http://www.agiledata.org/essays/tdd.html} (Test Driven Development).  Para ello utilizamos \htmladdnormallinkfoot{QuickCheck}{http://www.cs.chalmers.se/~rjmh/QuickCheck/} y desarrollamos una serie de tests que pueden ser ejecutados mediante el comando 
\lstset{language=sh, frame=single, tabsize=2}
\begin{lstlisting}
$ cabal test hpage
\end{lstlisting}
